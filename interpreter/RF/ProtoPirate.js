...
function durMatch(dur, target, delta) {
  return abs(dur - target) < delta;
}
  ...
  for (var i = 0; i < pulses.length; i++) {
    ...
    if (step === 0) {
      ...
      if (level) {
        ...
        if (
          durMatch(dur, p.te_short, p.te_delta) &&
          durMatch(teLast, p.te_short, p.te_delta)
        ) {
          headerCount++;
        } else if (
          durMatch(dur, p.te_long, p.te_delta) &&
          durMatch(teLast, p.te_long, p.te_delta)
        ) {
          if (headerCount > 15) {
            step = 2;
            dataHi = 0;
...
function extractKiaV0(dataHi, dataLo, bitCount) {
  ...
  return {
    ...
    serial: serial,
    button: button,
    btnName: getButtonName("Kia V0", button),
    counter: counter,
    crcOk: rxCrc === calcCrc,
  };
}
...
function loadAndDecodeFile(filepath) {
  ...
  try {
    ...
    if (!rawStr || rawStr.length < 10) {
      drawMessage("No RAW_Data!", RED);
      delay(1500);
      return false;
    }
    var pulses = parseRaw(rawStr);
    if (pulses.length < 20) {
      drawMessage("Not enough data!", RED);
      delay(1500);
      return false;
    }
    ...
    if (result) {
      ...
      drawMessage("Could not decode!", RED);
      delay(1500);
      return false;
    }
  } catch (e) {
...
function drawLoadMenu() {
  ...
  setTextSize(2);
  setTextColor(CYAN);
  drawString("Sub Decode", 10, 5);
  setTextSize(1);
  if (loadedFiles.length === 0) {
    setTextColor(RED);
    drawString("No .sub files found!", 10, 35);
    setTextColor(WHITE);
    drawString("Copy .sub files to", 10, 55);
    drawString("/subghz/ folder", 10, 70);
  } else {
    setTextColor(WHITE);
...
function drawHistoryMenu() {
  ...
  setTextSize(2);
  setTextColor(CYAN);
  drawString("Signal History", 10, 5);
  setTextSize(1);
  if (history.length === 0) {
    setTextColor(RED);
    drawString("No signals in history", 10, 35);
    setTextColor(WHITE);
    drawString("Capture a signal first", 10, 55);
  } else {
    setTextColor(WHITE);
    drawString("Last " + history.length + " of " + HISTORY_MAX, 10, 30);
    var y = 50;
  ...
  setTextColor(YELLOW);
  drawString("[PREV/NEXT] [SEL] View [ESC]", 5, screenHeight - 12);
}
...
function drawHistoryView() {
  ...
  for (var i = 0; i < lines.length; i++) {
    drawString(lines[i], 10, y);
    y += 12;
  }
  setTextColor(YELLOW);
  drawString("[PREV/NEXT] Navigate [ESC] Back", 5, screenHeight - 12);
}
...
function drawTimingAnalyzer() {
  ...
  setTextSize(2);
  setTextColor(CYAN);
  drawString("Timing Analyzer", 10, 5);
  setTextSize(1);
  setTextColor(WHITE);
  drawString("Capture a signal to", 10, 30);
  drawString("analyze timing", 10, 42);
  setTextColor(YELLOW);
  drawString("[SEL] Capture [ESC] Back", 5, screenHeight - 12);
}
...
function handleTimingAnalyzer() {
  ...
  if (getSelPress()) {
    ...
    if (rawContent && rawContent.length > 10) {
      ...
      drawMessage("Capture failed", RED);
      delay(1500);
    }
    drawTimingAnalyzer();
  }
...
function transmitSignal() {
  ...
  if (lastResult.encrypted) {
    ...
    setTextSize(2);
    setTextColor(RED);
    drawString("WARNING!", 10, 5);
    setTextSize(1);
    setTextColor(WHITE);
    drawString("ENCRYPTED SIGNAL", 10, 30);
    drawString("May desync your key!", 10, 45);
    setTextColor(GREEN);
    drawString("[SEL] Continue [ESC] Cancel", 5, screenHeight - 12);
    while (true) {
      if (getEscPress()) {
        drawResult(lastResult);
        return;
...
function drawMenu() {
  ...
  setTextSize(1);
  setTextColor(WHITE);
  drawString("Bruce Edition v3.0", 10, 28);
  drawStatusBar();
  var y = 50;
...
function drawResult(r) {
  ...
  setTextColor(WHITE);
  var y = 42;
  drawString("Key: " + toHex(r.dataHi, 8) + toHex(r.dataLo, 8), 10, y);
  y += 12;
  drawString("Sn:" + toHex(r.serial, 7) + " Btn:" + r.btnName, 10, y);
  y += 12;
  drawString("Cnt:0x" + toHex(r.counter, 4), 10, y);
  if (r.crcOk) {
    setTextColor(GREEN);
    drawString(" CRC:OK", 80, y);
  } else {
    setTextColor(RED);
    drawString(" CRC:FAIL", 80, y);
  }
  y = screenHeight - 55;
...
function drawInfo() {
  ...
  setTextColor(WHITE);
  var y = 28;
  drawString("Kia V0/V1/V2", 10, y);
  y += 10;
  drawString("Ford V0, Suzuki", 10, y);
  y += 10;
  drawString("StarLine (64-bit)", 10, y);
  y += 10;
  drawString("Scher-Khan", 10, y);
  y += 10;
  drawString("Subaru (64-bit)", 10, y);
  y += 10;
...
function drawFreqSelect() {
  ...
  setTextColor(YELLOW);
  drawString("[PREV/NEXT] [SEL] Set [ESC]", 5, screenHeight - 12);
}
...
function handleReceive() {
  ...
  if (rawContent && rawContent.length > 10) {
    ...
    setTextColor(YELLOW);
    drawString("[SEL] Decode [ESC] Menu", 5, screenHeight - 12);
    while (true) {
      if (getEscPress()) {
        setLongPress(true);
        appState = "menu";
    ...
    if (rawStr && rawStr.length > 10) {
      ...
      if (pulses.length > 20) {
        ...
        if (result) {
          ...
          setTextColor(WHITE);
          drawString("Freq: " + frequency + " MHz", 10, 50);
          setTextColor(YELLOW);
          drawString("[SEL] Save Raw [ESC] Menu", 5, screenHeight - 12);
          while (true) {
            if (getEscPress()) {
              setLongPress(true);
              appState = "menu";
            ...
            if (getSelPress()) {
              ...
              try {
                ...
                drawMessage("Saved!", GREEN);
                delay(1000);
              } catch (e) {
                drawMessage("Failed!", RED);
                delay(1000);
              }
              setLongPress(true);
              drawReceive();
